...
const EditPropertyForm = () => {
  ...
  const fetchProperty = async () => {
    try {
      const response = await axios.get(`${SERVER_URL}/api/properties/${id}`, {
        headers: {
      ...
      const selectedCities = citiesByRegion[property.location.region] || [];
      setCities(selectedCities);
      
      // Format the date for the input field and handle null values
      ...
      setFormData({
        ...
        profileImageUrl: getFullUrl(property.profileImage),
        bedroomImageUrl: getFullUrl(property.bedroomImage),
        bathroomImageUrl: getFullUrl(property.bathroomImage),
        otherImageUrl: getFullUrl(property.otherImage),
        ownershipDocumentUrl: getFullUrl(property.ownershipDocument)
      });
      
      setIsLoading(false);
    } catch (error) {
      console.error("Error fetching property:", error);
      alert(error.response?.data?.message || "Error loading property. Please try again.");
  ...
  const handleFileChange = (e) => {
    ...
    setFormData((prev) => ({ 
      ...prev, 
      [name]: file,
      [`${name}Url`]: file ? URL.createObjectURL(file) : prev[`${name}Url`]
    }));
    setErrors(prev => ({ ...prev, [name]: '' }));
  };
  ...
  const validateForm = () => {
    ...
    Object.keys(formData).forEach(key => {
      if (key === 'features' || key === 'amenities' || key.endsWith('Url')) {
        return;
      }
      ...
      if (typeof value === 'object' && value !== null && !(value instanceof File)) {
        Object.keys(value).forEach(subKey => {
          ...
          const error = validateField(fullKey, value[subKey]);
          if (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Append all files if they've been changed
    if (formData.profileImage instanceof File) data.append('profileImage', formData.profileImage);
    if (formData.bedroomImage instanceof File) data.append('bedroomImage', formData.bedroomImage);
    if (formData.bathroomImage instanceof File) data.append('bathroomImage', formData.bathroomImage);
    if (formData.otherImage instanceof File) data.append('otherImage', formData.otherImage);
    if (formData.ownershipDocument instanceof File) data.append('ownershipDocument', formData.ownershipDocument);
    
    // Append other form data with proper null handling
    data.append('title', formData.title);
    data.append('description', formData.description);
    data.append('listingType', formData.listingType);
    data.append('price', formData.price || 0);
    data.append('size', formData.size || 0);
    ...
    if (formData.availableFrom) {
      ...
      data.append('availableFrom', date.toISOString());
    } else {
      data.append('availableFrom', '');
    }
    ...
    try {
      await axios.put(`${SERVER_URL}/api/properties/${id}`, data, {
        headers: {
      ...
      if (error.response) {
        ...
        if (error.response.data.error && error.response.data.error.errors) {
          const serverErrors = {};
          Object.keys(error.response.data.error.errors).forEach(field => {
            serverErrors[field] = error.response.data.error.errors[field].message;
          });
  ...
  if (isLoading) {
    return <div className="max-w-3xl mx-auto p-6 text-center">Loading property data...</div>;
  }
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="col-span-2 border-t pt-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              {formData.profileImageUrl && !(formData.profileImage instanceof File) && (
                <div className="mb-2">
                  <img 
            ...
            <div>
              ...
              {formData.bedroomImageUrl && !(formData.bedroomImage instanceof File) && (
                <div className="mb-2">
                  <img 
                    src={formData.bedroomImageUrl} 
                    alt="Current bedroom" 
                    className="h-24 w-24 object-cover rounded"
                  />
                </div>
            ...
            <div>
              ...
              {formData.bathroomImageUrl && !(formData.bathroomImage instanceof File) && (
                <div className="mb-2">
                  <img 
                    src={formData.bathroomImageUrl} 
                    alt="Current bathroom" 
                    className="h-24 w-24 object-cover rounded"
                  />
                </div>
              )}
            ...
            <div>
              ...
              {formData.otherImageUrl && !(formData.otherImage instanceof File) && (
                <div className="mb-2">
                  <img 
        ...
        <div className="col-span-2">
          ...
          {formData.ownershipDocumentUrl && !(formData.ownershipDocument instanceof File) && (
            <div className="mb-2">
              <a 
                href={formData.ownershipDocumentUrl} 
                target="_blank" 
